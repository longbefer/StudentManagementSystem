...
<%
	...
	String speciality_id_new = request.getParameter("speciality_id_new");
	
	Speciality oldSpeciality = null;
	if (speciality_id != null && (!"".equals(speciality_id))) {
		oldSpeciality = new Speciality(Long.parseLong(speciality_id));
		oldSpeciality.setSpecialityInfo();
	}
	
	// 这里就不添加班级和老师对应对了，因为太难了。。。
	if (department_id != null && teacher_id != null && speciality_id_new != null &&
		speciality_name != null && (!"".equals(department_id)) && 
		(!"".equals(teacher_id)) && (!"".equals(speciality_id_new)) && 
		(!"".equals(speciality_name))) {
		
		// 若存在学生信息，则将其添加到数据库中
		StringBuffer message = new StringBuffer();
		
		Speciality speciality = new Speciality(Long.parseLong(speciality_id_new));
		...
		if (!un.updateSpeciality(oldSpeciality, speciality, message)) {
			out.println("<script type=\"text/javascript\">");
...
<body>
	<form action="modify_speciality.jsp">
		<input value="<%= oldSpeciality.id%>" style="display: none;" name="speciality_id"/>
		<table>
			<caption>修改专业</caption>
			<tbody>
				<tr>
					<td>专业号</td>
					<td><input type="text" placeholder="专业号" name="speciality_id_new" value="<%= oldSpeciality.id %>"/></td>
				</tr>
				<tr>
					<td>专业名</td>
					<td><input type="text" placeholder="专业名" name="speciality_name" value="<%= oldSpeciality.name%>"/></td>
				</tr>
				<tr>
					<td>指导老师</td>
					<td>
					...
					<%
						...
						for (Teacher tec : teachers) {
							// tec.setTeacherInfo();	
							if (tec.id == oldSpeciality.teacher.id) 
				...
				<tr>
					...
					<%
						...
						for (Department dep : depart) {
							...
							if (oldSpeciality.department.id == dep.id) 
								selected = "selected=\"selected\"";
							else selected = "";
					%>
							<option value="<%= dep.id %>" <%= selected %>><%= dep.id %> | <%= dep.name %></option>
					<%
						}
					%>
						</select>
